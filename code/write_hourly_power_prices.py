# File to write the hourly power prices
import pandas as pd
...
def write_hourly_power_prices(activities, output_name):
    ...
    df = pd.DataFrame(c)
    df.to_excel(output_name, sheet_name=sheet_name, header=False, index=False) # this approach makes sure that the sheet is closed after writing
